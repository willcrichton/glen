  ...
  function __construct($address,$port){
    ...
    while(true){
	  ...
	  foreach($this->sockets as $socket){
		$user = $this->getuserbysocket($socket);
		$this->send($user->socket,$this->buildPacket('test'));
		if(isset($user->needsOnConnect) and $user->needsOnConnect){ 
			$user->needsOnConnect = false;
			$this->onConnect($user);
		}
	  }
    }
